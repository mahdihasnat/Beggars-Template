...
struct Hashing{
	...
	void Build(string &str){Build1(str);Build2(str);}
	void Build1(const string &str) {
		int j = 0;
		for(ll i = str.size() - 1; i >= 0; i--){
			hsh[j][i] = fmod(hsh[j][i + 1],
													 bases[j], mods[j]) + str[i];
			if (hsh[j][i] > mods[j])
					hsh[j][i] -= mods[j];
		}
	}
	...
	pair<ll,ll> GetHash(ll i, ll j){
			assert(i <= j);
			ll tmp1 = (hsh[0][i] -
						fmod(hsh[0][j + 1] ,pwbase[0][j - i + 1]));
			ll tmp2 = (hsh[1][i] -
	fmod(hsh[1][j + 1] , pwbase[1][j - i + 1], mods[1]));
	...
	ll getSingleHash(ll i, ll j) {
		assert(i <= j);
		ll tmp1 = (hsh[0][i] - fmod(hsh[0][j + 1] ,
																pwbase[0][j - i + 1]));
		if(tmp1 < 0) tmp1 += mods[0];
		return tmp1;
	}

...
class SuffixAutomaton
{
    ...
    void process(int node)
    {
        ...
        if((int) st[node].next.size()<(int) alphabet.size()) st[node].shortest_non_appearing_string=1;
        for(mit=st[node].next.begin(); mit!=st[node].next.end(); ++mit)
        {
            ...
            st[node].length_of_substrings=st[node].length_of_substrings+st[mit->second].length_of_substrings+st[mit->second].substrings;
            st[node].shortest_non_appearing_string=min(st[node].shortest_non_appearing_string,1+st[mit->second].shortest_non_appearing_string);
        }
        if(st[node].link!=-1)
        {
...
int main()
{
    ...
    for(j=0; j<n; j++)
    {
        ...
        long long ans=sa.cyclic_occurrence(X);
        X.clear();
        printf("%I64d\n",ans);
    }
    return 0;
